        ...
        while q:
            ...
            for k in range(4):
                ...
                if 0 <= ni < K+K+N and 0 <= nj < K+K+M:
                    if cells[ni][nj] == 0:
                        cells[ni][nj] = [ni, nj, minV, life, minV+life+1]
                        re.append([ni, nj, minV, life, minV+life+1])
                        minVV = min(minVV,minV+life+1)
                    else:
                        if cells[ni][nj][2] == minV and cells[ni][nj][3] < life:  # 같은 시간에 들어오면
                            cells[ni][nj] = [ni, nj, minV, life, minV+life+1]
                            re.append([ni, nj, minV, life, minV+life+1])
                            minVV = min(minVV, minV+life+1)
        minV = minVV
...
for t in range(int(input())):
    ...
    cells = [[0]*(K+K+M) for _ in range(K)]+[[0]*K+list(map(int, input().split()))+[0]*K for _ in range(N)]+[[0]*(K+K+M)
                                                                                                             for _ in
                                                                                                             range(K)]
